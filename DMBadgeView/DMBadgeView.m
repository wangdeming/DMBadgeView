...
#error DMBadgeView must be compiled with ARC.
#endif
...
// Soon DMBadgeView will require iOS 7 and we'll be able to use the new methods.
#if  __IPHONE_OS_VERSION_MIN_REQUIRED >= __IPHONE_7_0
#define DMBadgeViewSilenceDeprecatedMethodStart()   _Pragma("clang diagnostic push") \
_Pragma("clang diagnostic ignored \"-Wdeprecated-declarations\"")
#define DMBadgeViewSilenceDeprecatedMethodEnd()     _Pragma("clang diagnostic pop")
#else
#define DMBadgeViewSilenceDeprecatedMethodStart()
#define DMBadgeViewSilenceDeprecatedMethodEnd()
#endif

static const CGFloat DMBadgeViewShadowRadius = 1.0f;
static const CGFloat DMBadgeViewHeight = 16.0f;
static const CGFloat DMBadgeViewTextSideMargin = 8.0f;
static const CGFloat DMBadgeViewCornerRadius = 10.0f;

// Thanks to Peter Steinberger: https://gist.github.com/steipete/6526860
static BOOL DMBadgeViewIsUIKitFlatMode(void)
{
...
@implementation DMBadgeView

+ (void)applyCommonStyle
{
    DMBadgeView *badgeViewAppearanceProxy = DMBadgeView.appearance;
    
    badgeViewAppearanceProxy.backgroundColor = UIColor.clearColor;
    badgeViewAppearanceProxy.badgeAlignment = DMBadgeViewAlignmentTopRight;
...
+ (void)initialize
{
    if (self == DMBadgeView.class)
    {
        [self applyCommonStyle];
        
        if (DMBadgeViewIsUIKitFlatMode())
        {
            [self applyIOS7Style];
        }
        else
...
- (void)layoutSubviews
{
    ...
    const CGFloat viewWidth = MAX(_badgeMinWidth, textWidth + DMBadgeViewTextSideMargin + (marginToDrawInside * 2));
    const CGFloat viewHeight = DMBadgeViewHeight + (marginToDrawInside * 2);
    ...
    switch (self.badgeAlignment) {
        case DMBadgeViewAlignmentTopLeft:
            newFrame.origin.x = -viewWidth / 2.0f;
            newFrame.origin.y = -viewHeight / 2.0f;
            break;
        case DMBadgeViewAlignmentTopRight:
            newFrame.origin.x = superviewWidth - (viewWidth / 2.0f);
            newFrame.origin.y = -viewHeight / 2.0f;
            break;
        case DMBadgeViewAlignmentTopCenter:
            newFrame.origin.x = (superviewWidth - viewWidth) / 2.0f;
            newFrame.origin.y = -viewHeight / 2.0f;
            break;
        case DMBadgeViewAlignmentCenterLeft:
            newFrame.origin.x = -viewWidth / 2.0f;
            newFrame.origin.y = (superviewHeight - viewHeight) / 2.0f;
            break;
        case DMBadgeViewAlignmentCenterRight:
            newFrame.origin.x = superviewWidth - (viewWidth / 2.0f);
            newFrame.origin.y = (superviewHeight - viewHeight) / 2.0f;
            break;
        case DMBadgeViewAlignmentBottomLeft:
            newFrame.origin.x = -viewWidth / 2.0f;
            newFrame.origin.y = superviewHeight - (viewHeight / 2.0f);
            break;
        case DMBadgeViewAlignmentBottomRight:
            newFrame.origin.x = superviewWidth - (viewWidth / 2.0f);
            newFrame.origin.y = superviewHeight - (viewHeight / 2.0f);
            break;
        case DMBadgeViewAlignmentBottomCenter:
            newFrame.origin.x = (superviewWidth - viewWidth) / 2.0f;
            newFrame.origin.y = superviewHeight - (viewHeight / 2.0f);
            break;
        case DMBadgeViewAlignmentCenter:
            newFrame.origin.x = (superviewWidth - viewWidth) / 2.0f;
            newFrame.origin.y = (superviewHeight - viewHeight) / 2.0f;
            break;
        default:
            NSAssert(NO, @"Unimplemented DMBadgeAligment type %lul", (unsigned long)self.badgeAlignment);
    }
...
{
    DMBadgeViewSilenceDeprecatedMethodStart();
    return [self.badgeText sizeWithFont:self.badgeTextFont];
    DMBadgeViewSilenceDeprecatedMethodEnd();
}

#pragma mark - Setters

- (void)setBadgeAlignment:(DMBadgeViewAlignment)badgeAlignment
{
...
- (void)drawRect:(CGRect)rect
{
    ...
    if (anyTextToDraw)
    {
        ...
        UIBezierPath *borderPath = [UIBezierPath bezierPathWithRoundedRect:rectToDraw byRoundingCorners:(UIRectCorner)UIRectCornerAllCorners cornerRadii:CGSizeMake(DMBadgeViewCornerRadius, DMBadgeViewCornerRadius)];
        ...
        CGContextSaveGState(ctx);
        {
            ...
            CGContextSetShadowWithColor(ctx, self.badgeShadowSize, DMBadgeViewShadowRadius, self.badgeShadowColor.CGColor);
